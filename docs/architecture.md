...
Operations do not register commands, directly mutate models or wrap external systems. Model changes remain the responsibility of controllers, and external systems are wrapped by services. Simple behaviour used by only one command should remain in its action rather than being converted into an operation by default.
