...
describe('ProjectClone', () => {
    ...
    describe('cloneRemoteProject', () => {
        ...
        it('creates task and runs clone command on valid https git URL', async () => {
            ...
            expect(vscode.Task).toHaveBeenCalledWith({ type: 'shell', taskId: `${manifest.PACKAGE_NAME} clone` }, expect.anything(), `Clone repo`, manifest.DISPLAY_NAME, expect.any(vscode.ShellExecution));
    ...
    describe('cloneLocalProject', () => {
        ...
        it('returns early when no project name provided', async () => {
            (vscode.workspace.workspaceFolders as any) = workspaceFolders;
            (vscode.window.showOpenDialog as jest.Mock).mockResolvedValueOnce([{ fsPath: '/path/to/source' }]);
            (vscode.window.showInputBox as jest.Mock).mockResolvedValueOnce(undefined);
        ...
        it('creates task and runs clone command for local path', async () => {
            ...
            topoCli.getCloneCommand.mockReturnValue(cloneCmd);
            (vscode.window.showOpenDialog as jest.Mock).mockResolvedValueOnce([{ fsPath: '/path/to/source' }]);
            (vscode.window.showInputBox as jest.Mock).mockResolvedValueOnce('myproj');

            await executeCommand(ProjectClone.localCloneCommand);

            expect(topoCli.getCloneCommand).toHaveBeenCalledWith(path.join(workspacePath, 'myproj'), { path: '/path/to/source', type: 'local' });
            expect(vscode.ShellExecution).toHaveBeenCalledWith('topo', 'clone myproj dir:/path/to/source'.split(' '), { cwd: workspacePath });
            expect(vscode.Task).toHaveBeenCalledWith({ type: 'shell', taskId: `${manifest.PACKAGE_NAME} clone` }, expect.anything(), `Clone myproj`, manifest.DISPLAY_NAME, expect.any(vscode.ShellExecution));
            expect(vscode.tasks.executeTask).toHaveBeenCalledWith(expect.objectContaining({ presentationOptions: expect.anything() }));
        });
        ...
        it('shows error when task ends with non-zero exit code', async () => {
            ...
            expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('Clone task "Clone myproj" failed with exit code 1.');
        });
