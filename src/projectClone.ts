...
const executeCloneTask = async (cloneCommand: string[], workspacePath: string, taskName: string): Promise<vscode.Disposable> => {
...
export class ProjectClone {
    ...
    public async activate() {
        this.context.subscriptions.push(
            ...
            vscode.commands.registerCommand(ProjectClone.localCloneCommand, this.cloneLocalProject.bind(this)),
        );
    }
    ...
    private async cloneLocalProject(): Promise<void> {
        ...
        const projectName = await vscode.window.showInputBox({ prompt: 'Enter the project name', value: path.basename(cloneSourcePath) });
        if (!projectName) {
            return;
        }
        const cloneCommand = this.getCloneCommand(workspacePath, projectName, cloneSourcePath, 'local');
        const taskName = `Clone ${projectName}`;
        try {
            const cloneTask = await executeCloneTask(cloneCommand, workspacePath, taskName);
    ...
    private async cloneRemoteProject(): Promise<void> {
        ...
        try {
            defaultProjectName = getDefaultProjectNameFromUrl(cloneSourceRemoteUrl);
        } catch {
        ...
        const cloneCommand = this.getCloneCommand(workspacePath, projectName, cloneSourceRemoteUrl, 'git');
        const taskName = `Clone ${projectName}`;
        try {
            const cloneTask = await executeCloneTask(cloneCommand, workspacePath, taskName);
            this.context.subscriptions.push(cloneTask);
    ...
    private getCloneCommand(workspacePath: string, projectName: string, cloneSourceLocation: string, type: CloneSource['type']): string[] {
